 ...
 * product sequences This class implements the Iterable<Integer> interface
 */
public class Sequence implements Iterable<Integer> {
	...
	/**
	 * constructor Creates a Sequence with respect to a user command line
	 * 
	...
	public Sequence(int[] command) {
		...
		case FIBONACCI:
			...
			break;
		case DIGIT_PRODUCT:
			DigitProductSequenceGenerator interator = new DigitProductSequenceGenerator(command[1], command[2]);
	...
	/**
	 * An accessor to the sequenceIterator field.
	 * 
	...
	private static boolean checkCommandSyntax(String[] userCommand) {
		// add the time complexity of this method, where the problem size N is the
		// length of the userCommand array provided as input argument parameter
		// time complexity: O(1)
	...
	public static void main(String[] args) {
		...
		boolean syntaxError = false; // boolean variable that tells if the syntax of the user command is correct
		Sequence sequence; // sequence object
		Scanner input = new Scanner(System.in); // Scanner object to read the user input from the keyboard
